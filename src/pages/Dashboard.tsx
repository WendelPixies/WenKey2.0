...
export default function Dashboard() {
      ...
      try {
        ...
        if (activeQuarter) {
          ...
          if (quarterResult && quarterResult.result_percent !== null) {
            setCurrentQuarterProgress(Math.round(quarterResult.result_percent));
          } else {
            const manual = await calculateUserQuarterProgress(
              selectedCompanyId,
              user.id,
              activeQuarter.id,
            );
            setCurrentQuarterProgress(manual);
          }

          const rankings = await calculateUserRankingsFromResults(selectedCompanyId, activeQuarter.id);
          setUserRankings(rankings);

          const objRanking = await calculateObjectiveRankings(selectedCompanyId, activeQuarter.id);
          setObjectiveRankings(objRanking);

          const okrRanking = await calculateOKRRankings(selectedCompanyId, activeQuarter.id);
        ...
        const perf = await calculateQuarterPerformanceFromResults(state, user.id);
  ...
  const calculateUserQuarterProgress = async (
    companyId: string,
    userId: string,
    quarterId: string
  ): Promise<number> => {
    ...
    if (!userObjectives || userObjectives.length === 0) return 0;
  ...
  const calculateQuarterPerformanceFromResults = async (
    ...
    for (const q of state.quarters) {
      ...
      if (qResult && qResult.result_percent !== null) {
        ...
        result_pct = await calculateUserQuarterProgress(state.company_id, userId, q.id);
      }
